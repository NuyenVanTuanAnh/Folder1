...
const MemberModal = ({
  ...
  const userRole =
    members.find((member) => member.email === userEmail)?.role || 0;
  ...
  const handleRemove = (email) => {
    if (!window.confirm(`Bạn có chắc chắn muốn xóa thành viên ${email}?`))
      return;
  ...
  const handleUpdateRole = (email, currentRole) => {
    ...
    if (!window.confirm(`Bạn có chắc chắn muốn ${action} quyền cho ${email}?`))
      return;
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {userRole === 2 && member.role !== 2 && (
                            <>
                              <button
                                onClick={() =>
                                  handleUpdateRole(member.email, member.role)
                                }
                                className={`p-2 ${
                                  member.role === 0
