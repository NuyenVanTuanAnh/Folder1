...
const FeedingTab = () => {
      ...
      callApi(
        ...
        (res) => {
          ...
          if (selectedPondOption) {
            ...
            setFormData((prev) => ({
              ...prev,
              pondId: selectedPondOption.value,
            }));
          }
        },
        null,
  ...
  const handleSubmit = useCallback(
    (e) => {
      ...
      if (
        !formData.pondId ||
        !formData.feedingDate ||
        feedingList.some((item) => !item.name || !item.amount)
      ) {
  ...
  const handleHistorySearch = useCallback(() => {
    ...
    callApi(
      [
        ShrimpRequestApi.ShrimpRequest.getFeedingFood(
          formData.pondId,
          formData.feedingDate
        ),
      ],
  ...
  useEffect(() => {
    if (
      location.state?.pondId &&
      location.state?.pondTypeId &&
      pondTypeOptions.length > 0
    ) {
