...
import HarvestRequest from "../../services/api/Harvest/HarvestRequestApi";
import MachineRequestApi from "../../services/api/MachineApi/MachineRequest";
...
function Card({
  ...
  const handleMachineStatusChanged = useCallback((data) => {
    setMachineData((prevData) => {
      ...
      const machineIndex = updatedData.findIndex(
        (m) => machineNameMapping[data.Name] === m.machineName
      );
    ...
    callApi(
      ...
      (res) => {
        const mappedData = res[0].machineDatas.map((machine) => ({
          ...machine,
          machineName:
            machineNameMapping[machine.machineName] || machine.machineName,
        }));
  ...
  const renderMachineStatus = () => {
    ...
    const machinesToShow = sortedMachines.slice(0, 3);
    const hasWasteSeparator = machinesToShow.some(
      (m) => m.machineName === "Máy lọc phân"
    );
    const displayMachines = machinesToShow.filter(
      (m) => m.machineName !== "Máy lọc phân"
    );
      ...
      <div className="p-3">
        <div className="grid grid-cols-2 gap-2">
          {displayMachines.slice(0, 2).map((machine, index) => (
            <motion.div
              ...
              animate={{
                ...
                scale:
                  updatedMachineId === machine.machineId ? [1, 1.05, 1] : 1,
              }}
              transition={{
                color: { duration: 0.3 },
                backgroundColor: { duration: 0.3 },
          ...
          {displayMachines.length < 2 &&
            Array(2 - displayMachines.length)
              .fill()
              .map((_, index) => (
                <div
                  key={`placeholder-${index}`}
        ...
        {(hasWasteSeparator || machinesToShow.length < 3) && (
          <motion.div
            className="mt-2 text-center py-2 text-sm font-medium rounded-md"
            animate={{
              backgroundColor: hasWasteSeparator
                ? machinesToShow.find((m) => m.machineName === "Máy lọc phân")
                    .machineStatus
                  ? "#DCFCE7"
                  : "#FEE2E2"
        ...
        {status ? (
          <div className="flex space-x-1 mx-auto">
            ...
            <ReactTooltip
              id={`envir-${pondId}`}
              place="top"
              content="Thông số môi trường"
            />
            <ReactTooltip
              id={`info-${pondId}`}
              place="top"
              content="Thông tin ao"
            />
            <ReactTooltip
              id={`delete-${pondId}`}
              place="top"
