...
function Register({ setIsRegister, onRegisterSuccess }) {
        ...
        (err) => {
          ...
          setErrorMessage(
            apiError?.description ||
              "Không thể đăng ký hoặc gửi OTP, vui lòng thử lại!"
          );
  ...
  const handleResendOtp = () => {
    ...
    callApi(
      ...
      (err) => {
        ...
        const apiError = err?.response?.data?.[0];
        setErrorMessage(
          apiError?.description || "Không thể gửi lại OTP, vui lòng thử lại!"
        );
  ...
  const handleOtpSubmit = (e) => {
    ...
    callApi(
      () =>
        DashboardRequestApi.authRequest.verifyEmail(
          email.trim(),
          otp.trim(),
          cacheKey
        ),
      ...
      (err) => {
        const apiError = err?.response?.data?.[0];
        setErrorMessage(
          apiError?.description || "Mã OTP không hợp lệ, vui lòng thử lại!"
        );
