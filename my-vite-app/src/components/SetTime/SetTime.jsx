        ...
        if (Array.isArray(res) && Array.isArray(res[0])) {
          const times = res[0].map((item) => {
            ...
            const closestMinute = minuteOptions.reduce((prev, curr) =>
              Math.abs(parseInt(curr) - parseInt(minute)) <
              Math.abs(parseInt(prev) - parseInt(minute))
                ? curr
                : prev
            );
  ...
  const handleAddTimeField = () => {
    ...
    setTimeout(() => {
      if (lastTimeFieldRef.current) {
        lastTimeFieldRef.current.scrollIntoView({
          behavior: "smooth",
          block: "nearest",
        });
      }
          ...
          <div className="max-h-64 overflow-y-auto mb-6 space-y-3 pr-2">
            {timeFields.length === 0 ? (
              <p className="text-center text-gray-500">
                Chưa có thời gian nào được thiết lập.
              </p>
            ) : (
              timeFields.map((time, index) => (
                <div
                  ...
                  ref={
                    index === timeFields.length - 1 ? lastTimeFieldRef : null
                  } // Gán ref cho mục cuối cùng
                >
