...
const TransferForm = () => {
  ...
  const fetchHarvestTime = useCallback(() => {
    ...
    callApi(
      [
        HarvestRequest.HarvestRequestApi.getHarvestTime(
          formData.transferPondId
        ),
      ],
      (res) => {
        console.log(res);
        setFormData((prev) => ({
          ...prev,
          harvestTime: String(res[0].harvestTime + 1),
        }));
      },
      null,
  ...
  const isFormValid = useCallback(() => {
    const { transferPondId, originPondId, transferDate, size, amount } =
      formData;
    return (
      transferPondId &&
      originPondId &&
  ...
  const handleSubmit = useCallback(
    (e) => {
      ...
      if (!isFormValid()) {
        setErrorMessage(
          "Vui lòng điền đầy đủ thông tin và đảm bảo giá trị hợp lệ!"
        );
        return;
      }
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-4xl mx-auto mt-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {errorMessage && (
          <p className="text-red-600 text-center text-lg animate-shake">
            {errorMessage}
          </p>
        )}
