...
import ConfigRequestApi from "../../services/api/ConfigApi/configRequest";
...
function Evista() {
    ...
    callApi(
      ...
      (res) => {
        ...
        setParameterLimits((prev) => ({
          ...
          Temperature: {
            min: config.temperatureLow,
            max: config.temperatureTop,
          },
        }));
        setTempLimits((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    if (selectedPond) {
      const pondExists = selectedPonds.some(
        (p) => p.value === selectedPond.value
      );
      if (!pondExists) {
    ...
    (pondTypeId, initialPondId) => {
      callApi(
        ...
        (res) => {
          ...
          setPondOptions(options);
          const selectedPondOption = options.find(
            (option) => option.value === initialPondId
          );
  ...
  useEffect(() => {
    if (location.state && pondTypes.length > 0) {
      ...
      const selectedPondTypeOption = pondTypes.find(
      ...
      parameters.forEach(({ name, data }) => {
        ...
        if (latestValue !== null && !meetThreshold(name, latestValue)) {
          ...
          const unit =
            name === "Temperature"
              ? "℃"
              : name === "O2" || name === "NH3" || name === "NO2"
              ? "mg/L"
              : "";
          toast.warn(
            `Thông số ${
              name === "Ph" ? "pH" : name
            } của ao ${pondName} (${latestValue}${unit}) ngoài ngưỡng cho phép (${
              limits.min
            } - ${limits.max}${unit}).`
          );
        }
  ...
  const handlePondTypeChange = (e) => {
    ...
    const selectedOption =
      pondTypes.find((option) => option.value === value) || null;
  ...
  const handlePondChange = (e) => {
    const value = e.target.value;
    const selectedOption =
      pondOptions.find((option) => option.value === value) || null;
    ...
    return selectedPonds.map((pond) => (
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-3xl font-semibold bg-teal-100 text-teal-700">
            {pond.label}
          </h2>
          <button
          ...
          {Object.keys(pondData[pond.value] || {}).map((param) => {
            const data = pondData[pond.value][param].map((d) => ({
              x: `${new Date(d.timestamp)
                .getDate()
                .toString()
                .padStart(2, "0")}/${(new Date(d.timestamp).getMonth() + 1)
                .toString()
                .padStart(2, "0")}-${new Date(d.timestamp).toLocaleTimeString(
                [],
            ...
            return (
              <div key={param} className="parameter-chart w-full">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-lg font-medium bg-teal-50 text-red-500">
                    {formattedParam}
                  </h3>
                  <button
                    onClick={() => {
                ...
                <Chart
                  ...
                  series={[
                    { name: formattedParam, data: data.map((d) => d.y) },
                  ]}
                  type="line"
                  height={250}
                  width="100%"
      ...
      <div className="flex-1 mt-16 sm:mt-0 overflow-y-auto overflow-hidden max-h-screen flex flex-col">
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 transition-all duration-300">
          ...
          <div className="max-w-8xl mx-auto z-10">
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 space-y-4 sm:space-y-0 hover:shadow-lg transition-all duration-300">
              ...
              <Chart
                options={{
                  ...
                  yaxis: {
                    title: {
                      text:
                        activeChart?.param === "Temperature"
                          ? "℃"
                          : activeChart?.param === "O2" ||
                            activeChart?.param === "NH3" ||
                            activeChart?.param === "NO2"
                          ? "mg/L"
                          : "",
                      style: {
                        fontSize: "14px",
                ...
                series={[
                  {
                    name: activeChart?.param,
                    data: activeChart?.data.map((d) => d.y),
                  },
                ]}
                type="line"
                height="90%"
