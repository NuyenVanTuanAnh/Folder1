...
import infoRequestApi from "../../services/api/InfoAPI/InfoRequest";
          ...
          <div className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-300">
              ...
              <div className="mt-6 space-y-6 overflow-y-auto sm:overflow-y-auto">
                <div className="bg-teal-50 p-4 rounded-lg">
                  ...
                  <p>
                    ...
                    {new Date(infoData.startDate).toLocaleDateString()}
                  </p>
                </div>
                    ...
                    <ul className="list-disc pl-5">
                      {infoData.certificates.map((cert, index) => (
                        <li key={index}>
                          ...
                          <a
                            href={`data:image/jpeg;base64,${cert.fileData}`}
                            className="text-blue-700 underline"
                            download={`${cert.certificateName}.jpg`}
                          >
                            Tải xuống
                          </a>
                        </li>
                      ...
                      <tbody>
                        {infoData.feedingFoods.map((food, index) => (
                          <tr key={index} className="border-t">
                            <td className="p-3">
                              {new Date(food.feedingDate).toLocaleDateString()}
                            </td>
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody>
                        {infoData.feedingMedicines.map((med, index) => (
                          <tr key={index} className="border-t">
                            <td className="p-3">
                              {new Date(med.feedingDate).toLocaleDateString()}
                            </td>
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody>
                        {infoData.sizeShrimps.map((size, index) => (
                          <tr key={index} className="border-t">
                            <td className="p-3">
                              {new Date(size.updateDate).toLocaleDateString()}
                            </td>
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody>
                        {infoData.lossShrimps.map((loss, index) => (
                          <tr key={index} className="border-t">
                            <td className="p-3">
                              {new Date(loss.updateDate).toLocaleDateString()}
                            </td>
            ...
            {!infoData && !isLoading && (
              <p className="text-teal-600 text-center py-4">
                Vui lòng chọn ao để xem thông tin
              </p>
            )}
          </div>
        </section>
