...
import MedicineRequestApi from "../../services/api/MedicineApi/medicineRequest";
...
const Food = () => {
  ...
  const handleDeleteFood = useCallback(
    (foodId, foodName) => {
      if (!window.confirm(`Bạn có chắc chắn muốn xóa thức ăn ${foodName}?`))
        return;
      setIsLoading(true);
      callApi(
  ...
  const handleDeleteMedicine = useCallback(
    (treatmentId, treatmentName) => {
      if (!window.confirm(`Bạn có chắc chắn muốn xóa thuốc ${treatmentName}?`))
        return;
      setIsLoading(true);
      callApi(
        ...
        () => {
          ...
          setTreatments(
            treatments.filter((treatment) => treatment.name !== treatmentName)
          );
        },
        (err) => {
          setIsLoading(false);
      ...
      callApi(
        ...
        (res) => {
          ...
          setFoods((prev) =>
            prev.map((food) =>
              food.name === foodName
                ? { ...food, name: editingFood.name.trim() }
                : food
            )
          );
          setEditingFood(null);
  ...
  const handleEditMedicine = useCallback(
    (e, treatmentName) => {
      ...
      const data = {
        medicineId: editingTreatment.medicineId,
          ...
          setTreatments((prev) =>
            prev.map((treatment) =>
              treatment.name === treatmentName
                ? { ...treatment, name: editingTreatment.name.trim() }
                : treatment
            )
          );
          ...
          <section>
            ...
            {foods.length === 0 ? (
              <p className="text-teal-600 text-center">
                Chưa có thức ăn nào trong danh sách
              </p>
            ) : (
                ...
                <table className="w-full min-w-[300px]">
                  ...
                  <tbody>
                    {treatments.map((treatment, index) => (
                      <tr
                        key={treatment.medicineId}
                        className="border-t hover:bg-teal-50 transition-all duration-150"
                      >
                        <td className="p-3 text-sm">{index + 1}</td>
                        {editingTreatment &&
                        editingTreatment.medicineId === treatment.medicineId ? (
                          <>
                            <td className="p-3">
                              <input
                                type="text"
                            ...
                            <td className="p-3 flex flex-col sm:flex-row gap-2">
                              <button
                                onClick={(e) =>
                                  handleEditMedicine(e, treatment.name)
                                }
                                className={`bg-teal-600 text-white px-3 py-1 rounded-lg transition-all duration-200 ${
                                  isLoading
                                    ? "opacity-50 cursor-not-allowed"
                          ...
                          <>
                            ...
                            <td className="p-3 flex flex-col sm:flex-row gap-2">
                              <button
                                onClick={() =>
                                  handleEditItem("treatments", treatment)
                                }
                                className="bg-amber-500 text-white px-3 py-1 rounded-lg hover:bg-amber-600 transition-all duration-200"
                              >
                                Sửa
