...
import AccessRequestApi from "../../services/api/AccessApi/AccessRequest";
...
const Access = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    callApi(
      [
        AccessRequestApi.AccessRequest.getAccessRequestBySeedId(
          selectedLot,
          selectedHarvestTime,
          farmId
        ),
  ...
  const downloadCertificateAsPDF = useCallback(
    (base64, index) => {
      const pdf = new jsPDF();
      pdf.addImage(base64, "JPEG", 10, 10, 180, 160);
      pdf.save(
        `Certificate_${selectedLot}_${selectedHarvestTime}_${index + 1}.pdf`
      );
    },
    [selectedLot, selectedHarvestTime]
  );
  ...
  const handleInputChange = useCallback(
    (field) => (e) => {
      ...
      setSelectedHarvestTime(
        field === "harvestTime" ? e.target.value : selectedHarvestTime
      );
      setData(null);
      setShowQRCode(false);
    },
                    ...
                    <ul className="list-disc pl-5">
                      {data.certificates.map((base64, index) => (
                        <li key={index}>
                          ...
                          <button
                            onClick={() =>
                              downloadCertificateAsPDF(base64, index)
                            }
