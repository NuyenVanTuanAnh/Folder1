...
const MachinesManager = () => {
  ...
  const handlePondChange = (selectedOptions) => {
    const selectedPonds = selectedOptions
  ...
  const handleSave = () => {
    const payload = {
      ...
      pondIds: tempPonds.map((pondId) => ({
        pondId: pondId,
        pondName:
          pondsOptions.find((pond) => pond.value === pondId)?.label || "",
      })),
    };
    setIsLoading(true);
    callApi(
      ...
      (err) => {
        setIsLoading(false);
        toast.error(
          "Lỗi khi cập nhật máy: " +
            (err?.response?.data?.title || "Thử lại sau!")
        );
      }
    );
  };
  ...
  const handleCreateMachine = () => {
    ...
    callApi(
      ...
      (err) => {
        toast.error(
          "Lỗi khi tạo máy: " + (err?.response?.data?.title || "Thử lại sau!")
        );
      }
    );
  };
  ...
  const handleDeleteMachine = (machineId) => {
    ...
    callApi(
      ...
      (err) => {
        setIsLoading(false);
        toast.error(
          "Lỗi khi xóa máy: " + (err?.response?.data?.title || "Thử lại sau!")
        );
      }
    );
  };
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-teal-50 to-gray-100">
      <Sidebar1></Sidebar1>
      <div className="w-full items-center justify-center p-10 m-4">
        <main>
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-sm font-semibold sm:text-sm text-black text-center">
                      Ao:{" "}
                      {machine.pondNames.length > 0
                        ? machine.pondNames.join(", ")
                        : "Chưa gắn"}
                    </p>
                  </div>
                </motion.div>
              ...
              <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 w-11/12 max-w-lg flex flex-col h-[400px]">
                ...
                <div className="flex-grow">
                  <Select
                    ...
                    value={pondsOptions.filter((pond) =>
                      tempPonds.includes(pond.value)
                    )}
